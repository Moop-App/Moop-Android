...
public class SettingsPresenter extends BasePresenter<SettingsContract.View>
    ...
    private Observable<SettingsViewState> getViewStateObservable() {
        return getFavoriteTheatersObservable()
                .map(DoneState::new);
    }

    private Observable<List<Theater>> getFavoriteTheatersObservable() {
        return Observable.just(theaterSetting.getFavoriteTheaters());
    }
}
